...
async def removejs(request):
    ...
    for i in l:
        ...
        if i['uid'] == t:
            ...
            await db_remove_print(app['engine'], t)
...
async def db_remove_print(engine, id):
    async with engine.acquire() as conn:
        await conn.execute(dm.tbl_print.delete().where(dm.tbl_print.c.uid == id))
    return 0
...
async def init(app):
    ...
    app.add_routes([
        # react용 리스트 반납
        web.get('/print/api/listjs', listjs),
        # 터미널용 리스트 반납
        web.get('/print/api/list', list),

        # react용 메모 추가
        web.post('/print/api/addjs', addjs),
        # 터미널용 메모 추가
        web.get('/print/api/add/{content:.*}', add),

        # react용 메모 제거
        web.post('/print/api/removejs', removejs),
        # 터미널용 메모 제거
        web.get('/print/api/remove', remove)
    ])
...
async def db_fetch_rows(lt, engine):
    ...
    # app['clipboards'] = []
    async with engine.acquire() as conn:
        async for r in conn.execute(dm.tbl_print.select()):
            dict = {}
            dict['uid'] = r.uid
            dict['font_size'] = r.font_size
            # dict['type'] = r.type
            dict['time'] = r.time
            dict['text'] = r.text
            dict['clone'] = r.clone
