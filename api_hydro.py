...
async def db_remove_memo(engine, id):
    async with engine.acquire() as conn:
        await conn.execute(dh.tbl_memo.delete().where(dh.tbl_memo.c.uid==id))
    return 0

async def db_add_memo(engine, dict):
    async with engine.acquire() as conn:
        await conn.execute(dh.tbl_memo.insert().values(time=dict['time'],
                                                    type=dict['type'],
                                                    uid=dict['uid'],
                                                    text=dict['text']))
    return 0
